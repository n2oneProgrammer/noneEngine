...
export default class ClippingPlane {
    ...
    clipTriangles(triangles: Array<Triangle>) {
        ...
        triangles.forEach((_t) => {
            result = result.concat(this.clipTriangle(_t.copy()));
        });
        return result;
    }
    ...
    clipTriangle(triangle: Triangle): Triangle[] {
        ...
        if (d0 >= 0 && d1 >= 0 && d2 >= 0) {
            ...
            return [new Triangle([triangle.vertices[1], _02, _12], triangle.normal,triangle.color), triangle];
        } else if (d0 > 0 && d1 < 0 && d2 > 0) {
            ...
            return [new Triangle([triangle.vertices[2], _01, _12], triangle.normal,triangle.color), triangle]
        } else if (d0 < 0 && d1 > 0 && d2 > 0) {
            const _01 = this.intersection(
                triangle.vertices[0],
                triangle.vertices[1]
            ...
            return [triangle, new Triangle([triangle.vertices[2], _01, _02], triangle.normal,triangle.color)]
        }
        return []
    }
}
